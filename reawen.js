...
client.on("userUpdate", async (eskiReaw, yeniReaw) => {

if (yeniReaw.bot) return;

if (eskiReaw.username !== yeniReaw.username) {
if (yeniReaw.username.includes("Pain") && !yeniReaw.roles.cache.has(conf.taglıRol) && yeniReaw.discriminator.includes("1986")) {
client.channels.cache.get(conf.tagLog).send(`${yeniReaw} isimli kullanıcı tagımızı aldı!`);
yeniReaw.roles.add(conf.taglıRol);
} else if (!yeniReaw.username.includes("Pain") && !yeniReaw.discriminator.includes("1986") && !yeniReaw.roles.cache.has(conf.taglıRol)) {
yeniReaw.roles.set([conf.kayıtsızRolü]);
client.channels.cache.get(conf.tagLog).send(`${yeniReaw} isimli kullanıcı tagımızı bıraktı!`);
}
}
})
...
client.on("userUpdate", async (reawEski, reawYeni) => {
let yasakliTag = await db.get(`yasakliTag`)
let logKanalID = conf.tagLog
let yasakliTagRolID = conf.yasakliTagRolID;
let kayitsizRolID = conf.kayıtsızRolü;

if (reawEski.username !== reawYeni.username) {
if (yasakliTag.some(asd => reawYeni.username.includes(asd))) {
reawYeni.roles.set([yasakliTagRolID]).catch(qwe => console.log(qwe));
client.channels.cache.get(logKanalID).send(`<@!${reawYeni.id}> isimli kullanıcı yasaklı taglardan birini kullanıcı adına eklediği için jaile atıldı!`)
reawYeni.send(`Yasaklı taglardan birine sahip olduğun için jaile atıldın. Çıkardıktan sonra tekrar sunucumuzda sohbete devam edebileceksin!\n\n\`>\` Sevgiler, **${reawYeni.guild.name}** Yönetim Ekibi\n\nYasaklı Taglar: ${yasakliTag.map(qwe => ` \`${qwe}\` `).join(", ")}`)
}
} else if (!yasakliTag.some(asd => reawYeni.username.includes(asd))) {
reawYeni.roles.set([kayitsizRolID]).catch(qwe => console.log(qwe));
client.channels.cache.get(logKanalID).send(`<@!${reawYeni.id}> isimli kullanıcı yasaklı tagı bıraktığı için kayıtsıza atıldı!`);
reawYeni.send(`Yasaklı tagımızı bıraktığın için jailden çıkarıldın. Yetkililer ile iletişime geçebilirsin dostum!\n\n\`>\` Sevgiler, **${reawYeni.guild.name}** Yönetim Ekibi`)
}
})